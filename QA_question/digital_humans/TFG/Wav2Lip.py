...
# import sys
# sys.path.append('..')
from digital_humans.src.models import Wav2Lip as wav2lip_mdoel
from digital_humans.src.utils import audio
...
class Wav2Lip:
    ...
    def predict(self, face, audio_file, batch_size):
        ...
        out.release()
        ans_path = "answer" + str(time.time()) + ".mp4"
        command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_file, 'temp/result.avi', 'results/' +
                                                                      ans_path)
